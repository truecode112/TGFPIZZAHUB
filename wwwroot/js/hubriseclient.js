...
function processOrder(formattedData, rawData) {
    try {
        ...
        var orderCard = `<summary class="card w-75 mt-2 flex-row" onClick="viewOrder(event, '${encoded}', '${order.id}', '${order.location_id}', '${expectTime}')" data-order-id="${order.id}"> 
        <div class="ml-3 w-10 flex-column d-flex align-items-center justify-content-center">
            <input type="text" class="timepicker timepicker-without-dropdown" tabindex="0" id="t-input-${order.id}" name="t-input-${order.id}"/>
        ...
        <div class="card-body"> 
            ...
            <p class="card-title mt-0 mb-0">${order.new_state.customer.first_name} ${order.new_state.customer.last_name}</p> 
            <p class="card-text mt-0 mb-1">${order.new_state.items.length} Product${order.new_state.items.length > 1 ? "s" : ""} (${order.new_state.total})</p> 
